...
const DEFAULT_HOST = process.env.CLAWPORT_HOST || '127.0.0.1'
...
function hostArgs() {
  const hasHost = extraArgs.some((a) => a === '--hostname' || a.startsWith('--hostname=') || a === '-H')
  if (hasHost) return []
...
function cmdDev() {
  ...
  run(NEXT_BIN, ['dev', ...hostArgs(), ...portArgs(), ...extraArgs])
}
...
function cmdStart() {
  ...
  build.on('close', (code) => {
    ...
    run(NEXT_BIN, ['start', ...hostArgs(), ...portArgs(), ...extraArgs])
  })
}
