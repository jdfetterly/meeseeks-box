...
function buildFetchMock() {
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/mobile/chat') && init?.method === 'POST') {
...
describe('mobile click contract', () => {
  ...
  it('sends mobile commands directly to OpenClaw instead of queueing a launch', async () => {
    const localStorageMock = installLocalStorageMock();
    localStorageMock.setItem('meeseeks-mobile.project-selection', JSON.stringify({ id: 'proj-2', title: 'Project Two' }));
    const fetchMock = buildFetchMock();
    vi.stubGlobal('fetch', fetchMock as unknown as typeof fetch);
    ...
    await waitFor(() => expect(screen.getByText('OpenClaw replied.')).toBeTruthy());

    const chatCall = fetchMock.mock.calls.find(
      ([url, init]) => String(url) === '/api/mobile/chat' && init?.method === 'POST',
    );
    ...
    expect(body).toMatchObject({
      ...
      message: 'Hey there, how you doing',
    });
    expect(fetchMock.mock.calls.some(([url]) => String(url) === '/api/product-state/launch')).toBe(false);
  });
