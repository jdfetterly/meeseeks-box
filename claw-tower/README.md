...
```bash
git clone https://github.com/jdfetterly/claw-tower.git
cd claw-tower
npm install
npm run setup
npm run dev
...
This fork is operated as `jdfetterly/claw-tower` with `JohnRiceML/clawport-ui` retained as read-only `upstream`.

- Agent contributions are branch-and-PR only.
- `openclaw-mini` must never merge to `main`.
- Human merge authority remains with `jdfetterly`.
- Loopback bind is the default runtime posture. Keep external access Tailnet-only by tunneling or explicitly allowlisting Tailnet origins.

Repo-local governance files live in `.github/workflows/`, `.github/pull_request_template.md`, and `tools/open_pr.py`.
